...
if TYPE_CHECKING:
    from custom_components.ha_aftership.config_flow_handler.options_flow import AftershipOptionsFlow

# Map exception types to error keys for user-facing messages
ERROR_MAP = {
    "AftershipApiClientAuthenticationError": "auth",
    "AftershipApiClientCommunicationError": "connection",
}


class AftershipConfigFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """
    Handle a config flow for ha_aftership.
    ...
    def async_get_options_flow(
        ...
        """
        from custom_components.ha_aftership.config_flow_handler.options_flow import (  # noqa: PLC0415
            AftershipOptionsFlow,
        )

        return AftershipOptionsFlow()
...
__all__ = ["AftershipConfigFlowHandler"]
