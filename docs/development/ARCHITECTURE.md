...
This document describes the technical architecture of the Aftership custom component for Home Assistant.

## Directory Structure

```text
custom_components/ha_aftership/
...
│   ├── __init__.py          # Exports AftershipDataUpdateCoordinator
...
│   ├── __init__.py          # Exports AftershipEntity
...
- `base.py` - Main coordinator class (`AftershipDataUpdateCoordinator`)
...
**Key class:** `AftershipDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `AftershipConfigFlowHandler` (main flow)
- `AftershipOptionsFlow` (options)
...
**Key class:** `AftershipEntity` (in `entity/base.py`)
...
2. `AftershipEntity` for common functionality
...
1. Create directory: `custom_components/ha_aftership/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `AftershipEntity`
