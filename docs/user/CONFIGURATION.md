...
This document describes all configuration options and settings available in the Aftership custom integration.
...
```yaml
service: ha_aftership.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: ha_aftership.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com/myztillx/ha-aftership/issues) - Report problems
